...
import type { ControlTreeExecutionRequest, ControlTreeExecutionResponse, TreeExecutionCommands } from '@/types/services/ControlTreeExecution';
...
<template>
    <button @click="() => controlExec(1)" class="btn btn-primary ms-1" title="Tick Once">
        <font-awesome-icon v-if="editor_store.running_commands.has(1)" icon="fas fa-check" spin class="show-button-icon"
            aria-hidden="true" />
    ...
    <button @click="() => controlExec(2)" class="btn btn-primary ms-1" title="Tick Periodically">
        <font-awesome-icon v-if="editor_store.running_commands.has(2)" icon="fas fa-sync" spin class="show-button-icon"
            aria-hidden="true" />
        <font-awesome-icon v-else icon="fas fa-sync" class="show-button-icon" aria-hidden="true" />
        <span class="ms-1 hide-button-text-control">
            Tick Periodically
        </span>
    </button>
    <button @click="() => controlExec(3)" class="btn btn-primary ms-1" title="Tick Until Result">
        <font-awesome-icon v-if="editor_store.running_commands.has(3)" icon="fas fa-play" spin class="show-button-icon"
            aria-hidden="true" />
    ...
    <button @click="() => controlExec(4)" class="btn btn-primary ms-1" title="Stop">
        <font-awesome-icon v-if="editor_store.running_commands.has(4)" icon="fas fa-stop" spin class="show-button-icon"
            aria-hidden="true" />
    ...
    <button @click="() => controlExec(5)" class="btn btn-primary ms-1" title="Reset">
        <font-awesome-icon v-if="editor_store.running_commands.has(4)" icon="fas fa-undo" spin class="show-button-icon"
            aria-hidden="true" />
    ...
    <button @click="() => controlExec(6)" class="btn btn-primary ms-1" title="Shutdown">
        <font-awesome-icon v-if="editor_store.running_commands.has(4)" icon="fas fa-power-off" spin class="show-button-icon"
            aria-hidden="true" />

...
import { SetOptionsRequest, SetOptionsResponse } from '../types/services/SetOptions';
import { RemoveNodeRequest, RemoveNodeResponse } from '../types/services/RemoveNode';
import { MorphNodeRequest, MorphNodeResponse } from '../types/services/MorphNode';
...
export class SelectedNode extends Component<SelectedNodeProps, SelectedNodeState> {
    ...
    nodeClassChangeHandler(event: ChangeEvent<HTMLSelectElement>) {
        ...
        if (flowControlNode && flowControlNode.length == 1) {
            var selectedFlowControlNode: DocumentedNode = flowControlNode[0];
            ...
            this.setState({
                ...
                inputs: this.getDefaultValues(selectedFlowControlNode.inputs,
                    selectedFlowControlNode.options),
                outputs: this.getDefaultValues(selectedFlowControlNode.outputs,
                    selectedFlowControlNode.options),
                isMorphed: true
            });
        }
    ...
    updateValue(paramType: string, key: string, new_value: string) {
        this.props.onNodeChanged(true);
        var map_fun = function (x: ParamData) {
            if (x.key === key) {
                return {
                    key: key,
                    value: {
        ...
        if (paramType.toLowerCase() === 'options') {
            ...
            this.setState(
                (prevState, _props) => {
                    ...
                    var resolve_refs = (current_item: ParamData) => {
                    ...
                    var newState: { options: ParamData[], inputs: ParamData[], outputs: ParamData[] } = { options: resolved_options, inputs: [], outputs: [] };

...
import type { SetExecutionModeRequest, SetExecutionModeResponse } from '@/types/services/SetExecutionMode';
...
function onClickStep() {
    ...
    ros_store.step_service.callService(
        {} as ContinueRequest,
...
<template>
    <div class="form-check m-1">
        <input type="checkbox" v-bind:id="debug_check_id" class="form-check-input" v-bind:checked="debugging"
            @change="handleDebugChange" />
        <label class="form-check-label" v-bind:for="debug_check_id">
            Debug
        </label>
    </div>
    <div class="form-check m-1">
        <input type="checkbox" v-bind:id="publish_subtrees_id" class="form-check-input" v-bind:checked="publish_subtrees"
            @change="handlePubSubtreesChange" />
        <label class="form-check-label" v-bind:for="publish_subtrees_id">
            Publish Subtrees
        </label>
    </div>
    <button @click="onClickStep" class="btn btn-primary ms-1" v-if="editor_store.debug">
        <font-awesome-icon icon="fa-solid fa-step-forward" aria-hidden="true" class="show-button-icon" />

...
import { ChangeTreeNameRequest, ChangeTreeNameResponse } from "../types/services/ChangeTreeName";
...
import { MigrateTreeRequest, MigrateTreeResponse } from "../types/services/MigrateTree";
import { GetPackageStructureRequest, GetPackageStructureResponse } from "../types/services/GetPackageStructure";
...
export class FileBrowser extends Component<FileBrowserProps, FileBrowserState> {
    get_package_structure_service?: ROSLIB.Service<GetPackageStructureRequest, GetPackageStructureResponse>;
    check_node_versions_service?: ROSLIB.Service<MigrateTreeRequest, MigrateTreeResponse>;
    migrate_tree_service?: ROSLIB.Service<MigrateTreeRequest, MigrateTreeResponse>;
    load_service?: ROSLIB.Service<LoadTreeRequest, LoadTreeResponse>;
    save_service?: ROSLIB.Service<SaveTreeRequest, SaveTreeResponse>;
    change_tree_name_service?: ROSLIB.Service<ChangeTreeNameRequest, ChangeTreeNameResponse>;
    ...
    searchPackageName(event: ChangeEvent<HTMLInputElement>) {
        if (this.props.packagesFuse) {
            var results = this.props.packagesFuse.search(event.target.value).map(x => x.item);
    ...
    render() {
        var comparePackageNames = function (a: Package, b: Package) {
        ...
        var comparePackageContent = function (a: PackageStructure, b: PackageStructure) {
        ...
        var package_results: Package[] = [];
        if (this.props.packagesFuse) {
            if (this.state.package_results.length === 0) {
                if (this.state.package === "") {
                    package_results = this.props.packagesFuse.search("").map(x => x.item).sort(comparePackageNames);
                }
            } else {
        ...
        var package_structure: JSX.Element | null = null;
            ...
            package_structure = (
                <div>
                    ...
                    <p>{tree!.children!.sort(comparePackageContent).map(child => {
                        var icon = (<i className="far fa-file"></i>);

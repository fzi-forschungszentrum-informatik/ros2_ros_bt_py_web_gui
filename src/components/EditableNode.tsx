...
import { GetMessageFieldsRequest, GetMessageFieldsResponse } from "../types/services/GetMessageFields";
import { DocumentedNode, Message, NodeData } from "../types/types";

import Fuse from "fuse.js";
import { getDefaultValue, getMessageType, getShortDoc, prettyprint_type, python_builtin_types, uuid } from "../utils";

interface ParamData {
    key: string, value: { type: string, value: any }
}
...
export class EditableNode extends Component<EditableNodeProps, EditableNodeState> {
    jsonRef: React.RefObject<unknown>;
    get_message_fields_service: ROSLIB.Service<GetMessageFieldsRequest, GetMessageFieldsResponse>;
    get_message_constant_fields_service: ROSLIB.Service<GetMessageFieldsRequest, GetMessageFieldsResponse>;
    ...
    renderSearchResults(results: Message[], key: string, onNewValue: (new_value: any) => void) {
    ...
    render() {
        var compareKeys = function (a: { key: string, value: { type: any, value: any } }, b: { key: string, value: { type: any, value: any } }) {
            if (a.key === b.key) {
                return 0;
            }
            if (a.key < b.key) {
        ...
        var node_class_name: string | JSX.Element = this.props.nodeClass;
        ...
        var doc_icon = null;
        if (this.props.doc) {
            doc_icon = (<i title={getShortDoc(this.props.doc)} className="fas fa-question-circle pl-2 pr-2"></i>);
        }
    ...
    handleOptionWirings(_paramType: string, key: string, new_value: string) {
        if (this.props.module === 'ros_bt_py.ros_nodes.enum' && this.props.nodeClass === 'Enum') {
            ...
            var referenced_node = this.props.availableNodes.filter((item) => {
    ...
    inputForValue(paramItem: ParamData, onValidityChange: (new_validity: boolean) => void, onNewValue: (new_value: any) => void) {
        var valueType = paramItem.value.type;
        var changeHandler = (_event: React.ChangeEvent<HTMLInputElement>) => { };
        var keyPressHandler = (_event: React.KeyboardEvent<HTMLInputElement>) => { };

        if (valueType === 'int') {
            // Number input with integer increments
            changeHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
                var newValue = Math.round(parseInt(event.target.value));
                if (isNaN(newValue)) {
                    newValue = 0;
                }
                onNewValue(newValue);
        ...
        if (valueType === 'float') {
            ...
            changeHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
                ...
                if (this.props.messagesFuse) {
                    ...
                    var message_results = results.slice(0, 5).map((x) => x.item)
                    this.setState({ messages_results: message_results, results_at_key: paramItem.key });
                }
                ...
                <div className="list-group-item"
                    key={name + x.key}>
                    {
                        this.inputForValue(
                            ...
                            (newVal: any) => this.updateValue(name, x.key, newVal))
                    }
                </div>
            );
        });
